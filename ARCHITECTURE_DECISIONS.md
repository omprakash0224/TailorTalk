...
*   **Model:** `openai/gpt-oss-120b or llama-3.3-70b-versatile` via **Groq**.
...
*   **Why Groq:** Groq's LPU inference engine delivers ultra-low latency, keeping the chat interface responsive during multi-turn shopping conversations.
...
1.  **FastAPI Core (`backend/main.py`):** Configures CORS for local development (ports 5173, 4173, 8000), includes chat routers, and exposes a `/health` endpoint. In production, the compiled React distribution (`frontend/dist`) can be served from the same container, but the current `main.py` does not mount static files directly — the Dockerfile copies the full project and `uvicorn` serves only the API.
2.  **Chat Router (`backend/routers/chat.py`):** Handles `POST /api/chat` supporting `multipart/form-data` for text messages combined with image files or URLs. Also exposes `GET /api/health`. Dispatches synchronous agent operations via `asyncio.to_thread` to keep the FastAPI event loop non-blocking.
...
*   **Two-Stage Dockerfile:**
    *   `Stage 1 (py-builder)`: Installs and compiles Python wheels.
    *   `Stage 2 (runtime)`: Debian slim runtime with OpenCV shared libraries (`libgl1`, `libglib2.0-0`, `curl`), exposing port 8000 and serving the unified API. The frontend `dist` is pre-built and copied in from the project source.
*   **Render Infrastructure (`render.yaml`):** Infrastructure-as-Code specification for deploying the Docker container with automatic health check monitoring at `/api/health` (router-level endpoint).

### Prototyping Runner
*   **Streamlit:** No longer present in the codebase. The project migrated fully to the FastAPI + React SPA architecture.
...
6.  **Rate-Limit Resilience:** Implements linear backoff retry handling (30 × attempt seconds) for Gemini API calls, up to 5 retries, to maintain stability under rate-limited conditions (`429`, `RESOURCE_EXHAUSTED`, `Quota exceeded`).
